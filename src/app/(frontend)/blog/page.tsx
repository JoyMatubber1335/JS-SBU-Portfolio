...
export default async function BlogPage() {
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-4">
        <div className="text-center max-w-2xl mx-auto">
          ...
          <p className="text-lg text-gray-700 mb-8">No blog posts found. Check back soon for new content.</p>
        </div>
      </div>
    )
  }
  ...
  const featuredPost = docs[0];
  const remainingPosts = docs.slice(1);
      ...
      <section className="py-20 px-4 bg-gradient-to-b from-gray-50 to-white">
        <div className="container mx-auto text-center max-w-4xl">
          ...
          <p className="text-xl text-gray-700 max-w-3xl mx-auto">
            Stay updated with the latest insights, technology trends, and expert perspectives from our team.
          </p>
        </div>
      </section>
                      ...
                      <div className="flex items-center">
                        <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center mr-3">
                          <span className="text-gray-600 font-semibold">
                            {typeof featuredPost.author === 'object' ? featuredPost.author.name.charAt(0) : 'A'}
                          </span>
                        </div>
                        <span className="text-sm text-gray-700 font-medium">
                          {typeof featuredPost.author === 'object' ? featuredPost.author.name : 'Author'}
                        </span>
                      </div>
                    )}
                  </div>
        ...
        <div className="container mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {remainingPosts.map((post, index) => (
              <div key={post.id} className="bg-white rounded-xl overflow-hidden shadow transition-all hover:shadow-lg flex flex-col h-full">
                {post.featuredImage && (
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  <Link href={`/blog/${post.slug}`} className="block flex-grow">
                    <h3 className="text-xl font-bold text-black mb-3 hover:text-blue-600 transition-colors">{post.title}</h3>
